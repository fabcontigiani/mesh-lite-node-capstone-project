...
/* Default number of frames to discard during warmup */
#define CAMERA_WARMUP_FRAMES 10

esp_err_t camera_warmup(int num_frames) {
#if ESP_CAMERA_SUPPORTED
  if (num_frames <= 0) {
    num_frames = CAMERA_WARMUP_FRAMES;
  }

  ESP_LOGI(TAG, "Warming up camera sensor (%d frames)...", num_frames);

  for (int i = 0; i < num_frames; i++) {
    camera_fb_t *fb = esp_camera_fb_get();
    if (fb == NULL) {
      ESP_LOGW(TAG, "Warmup frame %d failed", i + 1);
      continue;
    }
    ESP_LOGD(TAG, "Warmup frame %d/%d discarded (%zu bytes)", i + 1, num_frames,
             fb->len);
    esp_camera_fb_return(fb);
...
#if ESP_CAMERA_SUPPORTED
  /* First warm up the sensor */
  esp_err_t ret = camera_warmup(CAMERA_WARMUP_FRAMES);
  if (ret != ESP_OK) {
    ESP_LOGW(TAG, "Warmup failed, attempting capture anyway");
  }
